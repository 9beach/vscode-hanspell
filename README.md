...
`~/.hanspell-history` 파일에는 다음과 같이 교정 내용이 기록됩니다.
...
리눅스나 맥 환경이라면 다음과 같이 자주 틀리는 순으로 정렬할 수 있습니다.
...
어떤 문서에서 맞춤법 검사를 한번 실행하면, 다음 실행 전까지 동일한 오류에 한해서는 자동으로 교정 제안을 합니다. 해당 문서를 닫기 전에는 내부적으로 검사 결과를 계속 유지하기 때문입니다. 물론 새로운 오류를 찾기 위해서는 새로 맞춤법 검사를 실행해야 합니다.

같은 원리로, `~/.hanspell-typos` 파일에 다음과 같이 자주 틀리는 맞춤법을 미리 등록해서 자동 교정 제안에 활용할 수 있습니다. 단, 이 기능이 작동하려면 대상 문서에서 맞춤법 검사를 최소한 한번 실행해야 합니다.
...
다음은 자주 틀리는 맞춤법 20개로 `~/.hanspell-typos` 파일을 만드는 예입니다.

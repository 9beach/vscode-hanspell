...
만약 문서의 특정 블록만 명시적으로 선택되었다면 그 부분만 검사하니 주의하세요.

![commands](images/hanspell-commands.png)

맞춤법 검사를 마치면 의심되는 글자 아래에 붉은 밑줄이 생깁니다. 커서를 해당 글자로 옮기면 왼쪽에 녹색 전구가 생기고 마우스로 클릭하면 아래의 그림과 같이 추천 단어와 `맞춤법 오류 모두 교정` 메뉴가 나타납니다.

...
맞춤법 검사를 한번 실행하면, 다음 실행까지 동일한 오류에 한해서 자동으로 교정 제안을 합니다. 해당 문서를 닫기 전에는 내부적으로 검사 결과를 계속 유지하기 때문입니다. 물론 새로운 오류를 찾기 위해서는 새로 검사를 실행해야 합니다.

같은 원리로, `~/.hanspell-typos` 파일에 다음과 같이 자주 틀리는 맞춤법을 미리 등록해서 자동 교정 제안에 활용할 수 있습니다. 단, 이 기능을 쓰려면 대상 문서에서 최소한 한번 맞춤법 검사를 실행해야 합니다.

...
```json
{
  "bad-expressions": [
    ...
    {
      "expression": "([^ \n\t]+)의 +([^.]+)의((?=[^ㄱ-ㅎㅏ-ㅣ가-힣a-zA-Z])|$)",
      "info": "한 문장에서 ‘-의’가 두 번 이상 나옵니다. 자연스러운지 확인하세요.",
      "severity": "Information"
    },
    {
      "expression": "[^ \n\t]+[을를] +[^.]+[을를]((?=[^ㄱ-ㅎㅏ-ㅣ가-힣a-zA-Z])|$)",
      "info": "한 문장에서 ‘-을(를)’이 두 번 이상 나옵니다. 자연스러운지 확인하세요.",
      "severity": "Information"
    }
  ]
}
...
첫 번째 표현식은 “돈키호테” 좌우에 Lookahead and Lookbehind Zero-Length Assertions가 없어서 “돈키호테는”, “돈키호테여!” 등에 모두 적용될 수 있습니다.

두 번째 표현식은 “우리의 사랑의”와 같은 표현을 “우리 사랑의”로 고치도록 정의합니다.

세 번째, 네 번째 표현식은 마침표가 나오기 전에 ‘-의’ 또는 ‘-을(를)’이 각각 두 번 이상 나오면 알리도록 정의합니다.

`expression`은 반드시 정의해야 합니다. 나머지는 없어도 무방합니다. `severity`는 `Error`, `Warning`, `Information` 중에 하나를 고르세요.

형태소를 분석하지 않고 정규 표현식에 의존하는 것은 많은 한계가 있습니다. “세계의 불가사의”는 문제없는 표현이지만 위의 설정으로는 ‘-의’를 겹쳐 썼다고 분석합니다. 주의해서 사용하시기 바랍니다.

...
맞춤법 검사를 한 번이라도 실행한 뒤에는, 문서를 작성하다가 같은 실수를 반복하면 자동으로 밑줄이 생깁니다. 비주얼 스튜디오 코드를 새로 시작하기 전에는 검사 결과를 계속 유지하기 때문입니다. 물론 새로운 오류를 찾으려면 새로 검사해야 합니다.

이 원리를 이용해서, `~/.hanspell-typos` 파일에 아래의 예처럼 자주 틀리는 맞춤법을 미리 등록하여 자동 교정 제안에 활용할 수 있습니다. 단, 이 기능을 쓰려면 대상 문서에서 최소한 한번 맞춤법 검사를 실행해야 합니다.

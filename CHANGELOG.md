...
- 부산대학교 인공지능연구실/(주)나라인포테크의 맞춤법 검사기가 Cloudflare 봇 차단으로 외부 호출이 불가능해져 제거
- 대신 네이버(주) 맞춤법 검사기 추가: `맞춤법 검사 (네이버)` 커맨드와 `vscode-hanspell.spellCheckByNAVER` 명령 ID
- `맞춤법 검사 (다음, 부산대)` 커맨드는 `맞춤법 검사 (다음, 네이버)`로 변경
- `hanspell` 라이브러리 1.0.0 사용
- VS Code 최소 지원 버전을 1.85.0으로 상향
